...
int main(int argc, char *argv[]) {
    ...
    const int SERPENT_MAX  = 100;
    const int SERPENT_MIN  = 2;
    const int SDL_DELAY_MS = 10;

    int  largeur           =         entreeUser  (LARGEUR_MIN, LARGEUR_MAX);
    int  hauteur           =         entreeUser  (HAUTEUR_MIN, HAUTEUR_MAX);
    auto nbreSerpents      = size_t (entreeUser  (SERPENT_MIN, SERPENT_MAX));
    ...
    SDL_CreateWindowAndRenderer(largeur, hauteur, SDL_WINDOW_SHOWN, &window, &renderer);
    ...
    while (appIsRunning) {
        ...
        if (terrain.getnbrSerpents() <= 1) {
            cout << "Le serpent : " << vecSerpent.front().getSerpentID() << " a gagne !" << endl;
